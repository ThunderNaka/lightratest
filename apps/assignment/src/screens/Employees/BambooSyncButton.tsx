...
import { syncBambooEmployees } from "~/api/employees";
import { errorToast } from "~/utils";

export const BambooSyncButton = ({ lastSyncDate }: { lastSyncDate?: Date }) => {
  const queryClient = useQueryClient();
  const {
    mutate: syncBambooEmployeesMutation,
    isLoading,
    data,
  } = useMutation({
    mutationFn: syncBambooEmployees.mutation,
    onSuccess: () => syncBambooEmployees.invalidates(queryClient),
    onError: errorToast,
  });
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button
        onClick={() => syncBambooEmployeesMutation()}
        disabled={isLoading}
