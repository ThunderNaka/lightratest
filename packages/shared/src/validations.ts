...
const colorRegex = /[a-z][a-z][a-z]-\d00/i;
...
const dateVal = (isRequired = true) => {
  let schema = z
    ...
    .regex(
      /^$|([0-9]{2})\/([0-9]{2})\/([0-9]{2})/,
      "Invalid date. Format must be DD/MM/YY",
    );
  ...
  return schema.refine((v) => {
    ...
    const parsedDate = parse(v || "", "dd/MM/yy", new Date());
...
export const validations = {
  color: z.string().regex(colorRegex, "Value must be a valid Tailwind color"),
  date: dateVal,
  beforeDate: ({
    date = new Date(),
    req = true,
